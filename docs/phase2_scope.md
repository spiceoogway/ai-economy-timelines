# Phase 2 Scope: Fundamental Input Model
...
Phase 2 should **not** yet forecast capabilities. It should produce a defensible input-side compute capacity model that Phase 3 can allocate across training, inference, and AI R&D.
...
7. Do Phase 1 historical compute trends remain physically/economically plausible?
8. What input assumptions should be handed to Phase 3?
```
...
For Phase 2, you do **not** need perfect chip-by-chip precision. You need a common unit.
...
This matters because Phase 1 training-cost estimates are not the same as full infrastructure investment. Phase 1 found that cost variants diverge meaningfully and should be carried forward separately.
...
Phase 2 should preserve the Phase 1 cost-variant insight.
...
```text
Phase 2 estimates available compute capacity.
Phase 3 decides how that compute is allocated.
Phase 4 converts raw compute into effective compute.
Phase 5 maps effective compute to capabilities.
```
...
Phase 2 should produce yearly values from 2024–2040.
...
```text
data/assumptions/phase2_input_assumptions.csv
```
...
Phase 2 should include at least four scenarios.

## Scenario 1: Baseline Continuation

```text
Name: base_input_case
Purpose: central case using Phase 1 handoff assumptions
```

Use Phase 1 as an output check, not a direct driver.

Phase 1 handoff parameters:
...
```text
model/fundamental_inputs.py
```

Responsibilities:

```text
load_phase2_assumptions()
...
```text
scenarios/phase2_base_input_case.yaml
scenarios/phase2_chip_bottleneck.yaml
scenarios/phase2_power_datacenter_bottleneck.yaml
scenarios/phase2_capex_rich.yaml
```

---

## Deliverable 4: Phase 2 Notebook

```text
notebooks/06_phase2_fundamental_input_model.ipynb
```

Required sections:

```text
1. Load Phase 1 handoff parameters
2. Load Phase 2 assumptions
...
9. Compare implied compute growth vs Phase 1 historical trend
10. Export processed Phase 2 dataset
```
...
`phase2_findings.md` should end with handoff parameters for Phase 3:
...
Phase 2 is complete when the repo can:
...
6. Compare input-derived compute capacity against Phase 1 historical compute trends.
7. Export a clean annual dataset for Phase 3.
8. Document assumptions and uncertainties in a memo.
```

A strong Phase 2 should be able to defend statements like:

```text
“In the base input case, available AI compute grows X×/yr through 2030, below/above the Phase 1 historical frontier trend of ~6×/yr.”
...
“Under these assumptions, sustaining Phase 1-style compute growth would require approximately $X of cumulative AI infrastructure capex by year Y.”
```
...
Create a working Phase 2 skeleton that projects H100-equivalent stock and usable compute capacity under three scenarios.

## Sprint Tasks

```text
1. Add data/assumptions/phase2_input_assumptions.csv.
2. Add scenarios/phase2_base_input_case.yaml.
3. Add scenarios/phase2_chip_bottleneck.yaml.
4. Add scenarios/phase2_power_datacenter_bottleneck.yaml.
5. Implement model/fundamental_inputs.py.
...
10. Generate first comparison chart against Phase 1 Rule A historical trend.
```
...
The current repo structure already has `data`, `docs`, `notebooks`, `model`, `outputs/charts`, and `outputs/tables`.
...
data/
  assumptions/
    phase2_input_assumptions.csv

scenarios/
  phase2_base_input_case.yaml
  phase2_chip_bottleneck.yaml
  phase2_power_datacenter_bottleneck.yaml
  phase2_capex_rich.yaml

model/
  fundamental_inputs.py

notebooks/
  06_phase2_fundamental_input_model.ipynb
