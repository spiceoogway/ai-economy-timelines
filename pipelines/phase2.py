...
Outputs:
  ...
  outputs/tables/phase2_sensitivity_analysis.csv
...
from pipelines import phase2_charts as charts


def get_phase1_rule_a_2018_fit() -> tuple[float, float, float]:
    """Return (slope_log10_per_year, intercept_log10, annual_multiplier)
    for the Phase 1 Rule A 2018+ training-compute fit, with the
    intercept rebased so 2024 corresponds to 1e25 FLOP per single frontier run."""
...
def main() -> None:
    ...
    capex_df.to_csv(TABLES_DIR / "phase2_capex_requirements.csv", index=False)

    print("[3/8] Chart: accelerator stock...")
    charts.chart_accelerator_stock(
        df, scenarios, CHARTS_DIR / "phase2_accelerator_stock_h100e.png"
    )

    print("[4/8] Chart: theoretical + usable compute...")
    charts.chart_theoretical_and_usable_compute(
        df, scenarios, CHARTS_DIR / "phase2_theoretical_compute_capacity.png"
    )
    charts.chart_usable_compute_capacity(
        df, scenarios, CHARTS_DIR / "phase2_usable_compute_capacity.png"
    )

    print("[5/8] Chart: power capacity + capex required...")
    charts.chart_power_capacity_constraint(
        df, scenarios, CHARTS_DIR / "phase2_power_capacity_constraint.png"
    )
    charts.chart_capex_required(
        capex_df, scenarios, CHARTS_DIR / "phase2_capex_required.png"
    )

    print("[6/8] Chart: binding constraint heatmap...")
    years = sorted(df["year"].unique().tolist())
    charts.chart_binding_constraint_heatmap(
        df, scenarios, years, CHARTS_DIR / "phase2_binding_constraint_by_year.png"
    )

    print("[7/8] Chart: vs Phase 1...")
    p1_fit = get_phase1_rule_a_2018_fit()
    charts.chart_vs_phase1_compute_trend(
        df, scenarios, p1_fit, years, CHARTS_DIR / "phase2_vs_phase1_compute_trend.png"
    )

    print("[8/8] Chart: cost variants + sensitivity bands...")
    charts.chart_cost_per_h100e_by_variant(
        df, CHARTS_DIR / "phase2_cost_per_h100e_by_variant.png"
    )
    ...
    charts.chart_sensitivity_bands(
        ...
        CHARTS_DIR / "phase2_sensitivity_bands.png",
    )
    ...
    print(f"\n  P1 Rule A 2018+ historical multiplier: {p1_fit[2]:.2f}×/yr")
