# Phase 2 — Sprint 1 Initial Notes

**Date:** 2026-05-03
**Sprint goal:** working Phase 2 skeleton — accelerator stock, theoretical/usable compute, four scenarios, comparison chart vs Phase 1.

## What's in this sprint

- `data/assumptions/phase2_input_assumptions.csv` — long-format
...
- `scenarios/phase2_*.yaml` — 4 scenario configs (`base_input_case`, `chip_bottleneck`, `power_datacenter_bottleneck`, `capex_rich`).
- `model/fundamental_inputs.py` — projection pipeline:
...
- `notebooks/run_phase2_sprint1.py` — driver running all 4 scenarios.
- Outputs:
...
**Every numeric input in `phase2_input_assumptions.csv` is a sprint-1 placeholder.**
...
For comparison, Phase 1 Rule A 2018+ historical frontier-compute trend
...
### 1. The Phase 1 6×/yr extrapolation is unphysical under any of our placeholder scenarios.
...
total usable compute grows at ~42%/yr. That is one quarter of the Phase 1
rate. By 2040 the gap between the historical extrapolation and Phase 2
supply is **~10 OOM**.
...
- **The historical rate is the rate of single-frontier-runs**, not total
  ...
  shipments. Phase 3 (allocation) is where this matters.
- **The historical 6× cannot continue forever** anyway. Even ignoring our
  ...
  fundamentals start binding — Phase 2's job.
...
4. **Add hardware-performance index time series.**
   ...
   improvement (H100→B200→Rubin→…) so Phase 3 can use it.
...
7. **Connect to Phase 1 cost-variant insight.**
   Compute and report `cost_per_h100e_year` in the three Phase 1 cost
   variants (upfront / cloud / blended) — currently we only emit a single
   amortized figure.

## Open questions for the user (Phase 2 only)

1. **Geography**: model global aggregate, or split US / China / RoW? Phase
   1 was global; Phase 2 placeholders are global. Splitting matters more
...
2. **Time grain**: annual is fine for capacity; should we go to quarterly
   for the 2024-2027 calibration window? Phase 1 had publication-date
